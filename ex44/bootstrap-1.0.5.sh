...
#   curl -sLO https://raw.githubusercontent.com/jedarden/bootstrap/main/ex44/bootstrap-1.0.5.sh
#   chmod +x bootstrap-1.0.5.sh
#   ./bootstrap-1.0.5.sh
...
if [[ -n "$B2_BUCKET" && -n "$B2_ACCOUNT_ID" ]]; then
    ...
    if [[ -n "$B2_APPLICATION_KEY" ]]; then
        ...
        # Unset secrets only (B2_ACCOUNT_ID is not sensitive, keep it for save_config)
        unset B2_APPLICATION_KEY RESTIC_PASSWORD RESTIC_REPOSITORY
    else
