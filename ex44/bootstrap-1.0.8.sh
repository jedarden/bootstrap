...
#   curl -sLO https://raw.githubusercontent.com/jedarden/bootstrap/main/ex44/bootstrap-1.0.8.sh
#   chmod +x bootstrap-1.0.8.sh
#   ./bootstrap-1.0.8.sh
...
# Detect IPv6-only (no IPv4 global address)
IPV6_ONLY=false
if ! ip -4 addr show scope global 2>/dev/null | grep -q inet; then
    IPV6_ONLY=true
    echo "Detected IPv6-only server - will use Hetzner DNS64 for NAT64"
fi
